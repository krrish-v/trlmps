...
# Default chunk size optimized for M4 Pro's memory bandwidth.
# 65536 reduces Python-to-Metal dispatch overhead from 19 to 3 iterations
# for 152K vocab (Qwen). Measured 2.5× speedup over 8192.
...
@_compile_decorator
def _chunked_forward_pass_pass1(hidden_states, weight, bias, running_max, running_sum_exp, V, chunk_size):
    for v_start in range(0, V, chunk_size):
        ...
        chunk_max = chunk_logits_f.max(dim=-1, keepdim=True).values
        new_max = torch.maximum(running_max, chunk_max)

        running_sum_exp = (
            running_sum_exp * torch.exp(running_max - new_max)
            + torch.exp(chunk_logits_f - new_max).sum(dim=-1, keepdim=True)
        )
        running_max = new_max
    return running_max, running_sum_exp

@_compile_decorator
def _chunked_forward_pass_pass2(hidden_states, weight, bias, target_logits, labels, valid_mask, V, chunk_size):
    for v_start in range(0, V, chunk_size):
        v_end = min(v_start + chunk_size, V)
        ...
        gathered = chunk_logits.float().gather(-1, local_labels.unsqueeze(-1)).squeeze(-1)
...
@_compile_decorator
def _chunked_backward_pass(hidden_states, weight, bias, labels, valid_mask, scale, logsumexp, V, chunk_size, grad_hidden, grad_weight, grad_bias):
    B, T, D = hidden_states.shape
    for v_start in range(0, V, chunk_size):
        v_end = min(v_start + chunk_size, V)
        chunk_weight = weight[v_start:v_end]

        chunk_logits = torch.matmul(hidden_states, chunk_weight.t())
        if bias is not None:
            chunk_logits = chunk_logits + bias[v_start:v_end]

        chunk_probs = torch.exp(chunk_logits.float() - logsumexp.unsqueeze(-1))
        grad_logits = chunk_probs.clone()

        # DENSE Mapping: branchless execution without .any() guarantees zero pipeline stalls
        chunk_classes = torch.arange(v_start, v_end, device=labels.device, dtype=labels.dtype).view(1, 1, -1)
        is_correct_class = (labels.unsqueeze(-1) == chunk_classes)
        grad_logits = torch.where(is_correct_class, grad_logits - 1.0, grad_logits)

        grad_logits = grad_logits * valid_mask.unsqueeze(-1).float() * scale

        grad_hidden += torch.matmul(grad_logits, chunk_weight)
        grad_weight[v_start:v_end] += torch.matmul(
            grad_logits.reshape(-1, v_end - v_start).t(),
...
class ChunkedFusedLinearCrossEntropy(torch.autograd.Function):
    ...
    def forward(
        ...
        # ------------------------------------------------------------------
        # CRITICAL: All accumulators run in FP32 to prevent bf16 overflow.
        # With 152K vocab and chunk_size=65536, sum_exp can reach ~65K which
        # exceeds bf16 max (65504). FP32 handles up to 3.4e38.
        ...
        running_max, running_sum_exp = _chunked_forward_pass_pass1(
            hidden_states, weight, bias, running_max, running_sum_exp, V, chunk_size
        )
        ...
        target_logits = torch.zeros(B, T, device=hidden_states.device, dtype=torch.float32)

        target_logits = _chunked_forward_pass_pass2(
            hidden_states, weight, bias, target_logits, labels, valid_mask, V, chunk_size
        )
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        grad_hidden, grad_weight, grad_bias = _chunked_backward_pass(
            hidden_states, weight, bias, labels, valid_mask, scale, logsumexp, 
            V, chunk_size, grad_hidden, grad_weight, grad_bias
        )
