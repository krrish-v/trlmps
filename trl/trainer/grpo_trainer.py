...
from .mps_utils import is_mps_available, mps_sync_and_clear, mps_memory_guard, get_mps_device_info, mps_aggressive_cleanup
from .utils import (
    RepeatSampler,
...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if self._use_mps:
            ...
            logger.info(
                f"GRPO MPS optimization enabled: sync barriers after generation and logps. "
            ...
            # Compiles each transformer layer.forward + Vision Merger individually.
            # LoRA adapters traced as pure linear algebra — no graph breaks.
            # dynamic=False: prevents silent recompile on variable VLM image sizes.
            ...
            if getattr(args, "use_metal_liger_compile", False):
                try:
                    ...
                    logger.info(
                        ...
                        f"{'Vision Merger' if result['merger_compiled'] else 'no merger'}. "
                        f"Warmup (fwd+bwd) fires on first training step."
                    )
                except Exception as e:
    ...
    def get_eval_dataloader(self, eval_dataset=None):
        """Override eval dataloader for MPS: force num_workers=0 to prevent IPC overhead.

        During eval, the GPU has no backward pass and processes batches ~25x faster
        than training. Background workers add IPC serialization overhead that starves
        the GPU. mlx-data's Buffered pattern shows that direct feeding (0 workers)
        eliminates this bottleneck when the consumer is fast.
        """
        if getattr(self, '_use_mps', False):
            ...
            self.args.dataloader_num_workers = getattr(self.args, 'mps_eval_num_workers', 0)
            # PyTorch rejects prefetch_factor when num_workers=0
            if self.args.dataloader_num_workers == 0:
                self.args.dataloader_prefetch_factor = None
            result = super().get_eval_dataloader(eval_dataset)
    ...
    def _get_per_token_logps_and_entropies(
        ...
        When MPS fused CE is active, uses chunked computation that avoids
        materializing the full [B, T, V] logit tensor (~740MB savings).
        """
        # Check if we can use the MPS chunked path
        _use_chunked = getattr(self, "_use_mps", False) and getattr(self.args, "use_metal_liger", False)

        if _use_chunked:
            return self._chunked_per_token_logps_and_entropies(
                model, input_ids, attention_mask, logits_to_keep,
                batch_size=batch_size, compute_entropy=compute_entropy,
                pixel_values=pixel_values, image_grid_thw=image_grid_thw,
                num_images=num_images, pixel_attention_mask=pixel_attention_mask,
                image_sizes=image_sizes, token_type_ids=token_type_ids,
            )

        # Standard path: materialize full logits
    ...
    def _chunked_per_token_logps_and_entropies(
        self,
        model,
        input_ids,
        attention_mask,
        ...
        """MPS-optimized: compute per-token logps WITHOUT materializing [B, T, V] logits.

        Strategy:
          1. Run model backbone to get last_hidden_state [B, T, H]
          2. For each vocab chunk [chunk_size]:
             - Compute partial logits: hidden @ weight_chunk.T → [B, T, chunk_size]
             - Online logsumexp for normalization
             - Gather selected token logit (if in this chunk)
          3. Per-token logps = selected_logit - logsumexp
          4. Entropy computed from chunked softmax probabilities

        Peak memory: [B, T, chunk_size] instead of [B, T, V] (~740MB savings for 152K vocab).
        """
        chunk_size = getattr(self.args, "mps_fused_loss_chunk_size", 65536)
        unwrapped_model = self.accelerator.unwrap_model(model)

        # Get last_hidden_state from backbone (matches existing Liger path)
        hidden = self._get_last_hidden_state(
            unwrapped_model, input_ids, attention_mask, logits_to_keep,
            pixel_values=pixel_values, image_grid_thw=image_grid_thw,
            pixel_attention_mask=pixel_attention_mask, image_sizes=image_sizes,
        )  # [B, logits_to_keep, H]

        # Get lm_head weight (handles PEFT unwrapping)
        if is_peft_model(unwrapped_model):
            base_model = unwrapped_model.base_model.model
        else:
            base_model = unwrapped_model
        lm_head_weight = base_model.lm_head.weight  # [V, H]
        lm_head_bias = getattr(base_model.lm_head, "bias", None)

        V = lm_head_weight.shape[0]
        B, T, H = hidden.shape
        completion_ids = input_ids[:, -logits_to_keep:]  # [B, T]

        # Apply temperature
        inv_temp = 1.0 / self.temperature if self.temperature != 1.0 else 1.0

        hidden_f = hidden.float()
        weight_f = lm_head_weight.float()

        # ── Pass 1: Chunked online logsumexp + selected token logit ──
        running_max = torch.full((B, T), float('-inf'), device=hidden.device, dtype=torch.float32)
        running_sum_exp = torch.zeros(B, T, device=hidden.device, dtype=torch.float32)
        selected_logits = torch.zeros(B, T, device=hidden.device, dtype=torch.float32)

        # For entropy: accumulate weighted log-prob sum
        if compute_entropy:
            entropy_accum = torch.zeros(B, T, device=hidden.device, dtype=torch.float32)
            # We need a second pass for entropy since it requires normalized probabilities.
            # Save chunk_logits info for the second pass.

        for v_start in range(0, V, chunk_size):
            v_end = min(v_start + chunk_size, V)
            w_chunk = weight_f[v_start:v_end]  # [chunk, H]
            b_chunk = lm_head_bias[v_start:v_end].float() if lm_head_bias is not None else None

            chunk_logits = torch.nn.functional.linear(hidden_f, w_chunk, b_chunk)  # [B, T, chunk]
            chunk_logits = chunk_logits * inv_temp

            # Online logsumexp update
            chunk_max = chunk_logits.max(dim=-1).values  # [B, T]
            ...
            # Gather selected token logit if it falls in this chunk
            in_range = (completion_ids >= v_start) & (completion_ids < v_end)
            if in_range.any():
                local_ids = (completion_ids - v_start).clamp(min=0, max=v_end - v_start - 1)
                gathered = chunk_logits.gather(-1, local_ids.unsqueeze(-1)).squeeze(-1)
                selected_logits = selected_logits + gathered * in_range.float()

            del chunk_logits

        # log_softmax(selected) = selected_logit - logsumexp
        logsumexp = running_max + running_sum_exp.log()
        per_token_logps = (selected_logits - logsumexp).to(hidden.dtype)  # [B, T]

        # ── Entropy (optional, second pass) ──
        entropies = None
        if compute_entropy:
            with torch.no_grad():
                entropy_accum = torch.zeros(B, T, device=hidden.device, dtype=torch.float32)
                for v_start in range(0, V, chunk_size):
                    v_end = min(v_start + chunk_size, V)
                    w_chunk = weight_f[v_start:v_end]
                    b_chunk = lm_head_bias[v_start:v_end].float() if lm_head_bias is not None else None
                    chunk_logits = torch.nn.functional.linear(hidden_f, w_chunk, b_chunk) * inv_temp
                    # Softmax normalization using global logsumexp
                    chunk_log_probs = chunk_logits - logsumexp.unsqueeze(-1)  # [B, T, chunk]
                    chunk_probs = chunk_log_probs.exp()
                    # Entropy contribution: -sum(p * log_p)
                    entropy_accum = entropy_accum - (chunk_probs * chunk_log_probs).sum(dim=-1)
                    del chunk_logits, chunk_log_probs, chunk_probs
                entropies = entropy_accum.to(hidden.dtype)
    ...
    def _generate_and_score_completions(
        ...
        # mlx-data DynamicBatch pattern: adjust sub-batch size based on token budget.
        # Instead of fixed batch_size for the forward pass sub-batching, compute how many
        # samples fit within the token budget given the actual sequence length.
        # Shorter sequences → larger sub-batches, longer sequences → smaller sub-batches.
        # Total memory per sub-batch stays bounded → no RAM spikes during scoring.
        mps_max_tokens = getattr(self.args, 'mps_max_tokens_per_batch', None)
        if self._use_mps and mps_max_tokens is not None:
            seq_len = prompt_completion_ids.size(1)  # P+C
            dynamic_batch_size = max(1, mps_max_tokens // seq_len)
            if dynamic_batch_size < batch_size:
                logger.info(
                    f"MPS DynamicBatch: seq_len={seq_len}, reducing sub-batch from "
                    f"{batch_size} to {dynamic_batch_size} (budget={mps_max_tokens} tokens)"
                )
        ...
        # Decode
        # SYNC POINT 3: After generation + scoring — the heaviest memory point in GRPO.
        # Uses aggressive cleanup throttled by mps_cleanup_frequency config.
        # gc.collect only every N steps (default 4), sync+clear every step.
        if self._use_mps:
            mps_aggressive_cleanup(every_n_steps=self._mps_cleanup_freq)
        ...
        # SYNC POINT 5 (mlx-data deterministic cleanup pattern):
        # At this point, all intermediate scoring tensors have been consumed into
        # the output dict or logged. On CUDA these would be lazily freed, but on
        # MPS they linger in the command buffer, contributing to RAM/swap pile-up.
        # Explicitly delete the heaviest intermediates and sync before returning.
        if self._use_mps:
            del rewards_per_func, prompts_text, completions_text
            if 'all_process_advantages' in dir():
                del all_process_advantages
            mps_sync_and_clear()
    ...
    def compute_liger_loss(self, unwrapped_model, inputs):
        ...
        mode = "train" if self.model.training else "eval"
        if self.beta != 0.0:
            self._metrics[mode]["kl"].append(self.accelerator.gather(mean_kl).mean().detach().item())
        self._metrics[mode]["clip_ratio"].append(self.accelerator.gather(clip_ratio).mean().detach().item())
    ...
    def _compute_loss(self, model, inputs):
        ...
        # SYNC POINT 4: After per-token logps computation — free intermediate
        # activations before computing the policy gradient loss. This prevents
        # the logits/softmax tensors from lingering while loss computation
        ...
        if self.beta != 0.0:
            ...
            self._metrics[mode]["kl"].append(self.accelerator.gather(mean_kl).nanmean().detach().item())

        mean_entropy = masked_batch_mean(entropies)
        self._metrics[mode]["entropy"].append(self.accelerator.gather(mean_entropy).nanmean().detach().item())
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys: list[str] | None = None):
        ...
        if getattr(self, "_use_mps", False):
            ...
            mps_aggressive_cleanup(
                every_n_steps=getattr(self, "_mps_cleanup_freq", 4),
