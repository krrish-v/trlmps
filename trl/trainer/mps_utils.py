...
# Tracks how often deep cleanup runs
_cleanup_step_counter: int = 0


def _background_gc(triple: bool = False):
    """Run gc.collect in current thread (called from daemon thread)."""
    if triple:
        gc.collect()
        gc.collect()
        gc.collect()
        # Clear uncollectable cycles that accumulate with autograd __del__ methods
        gc.garbage.clear()
    else:
        gc.collect()
...
def mps_aggressive_cleanup(every_n_steps: int = 1, deep_clean_interval: int = 100):
    """Two-tier memory cleanup for MPS to prevent gradual RAM growth.

    **Critical optimization**: ``gc.collect()`` runs in a background daemon
    thread so the main thread can immediately grab the next batch from
    DataLoader workers. This eliminates 50-100ms of GPU idle time per step.

    The GPU sync (``synchronize`` + ``empty_cache``) still runs on the main
    thread since those are fast (~5ms) and must complete before the next
    forward pass writes into the same memory.

    **Tier 1 (every step)**: Background ``gc.collect()`` + main-thread sync.
    **Tier 2 (every ``deep_clean_interval`` steps)**: Triple ``gc.collect()``
    + ``gc.garbage.clear()`` in background, plus memory logging.

    Args:
        every_n_steps: Throttle tier-1 gc.collect (default 1 = every step).
            Only increase if you're sure memory is stable.
        deep_clean_interval: Steps between deep cleans (default 100).
            Set lower (e.g. 50) on 16GB Macs.
    ...
    global _cleanup_step_counter
    _cleanup_step_counter += 1

    is_deep_clean = (_cleanup_step_counter % deep_clean_interval == 0)

    # Always: Sync GPU first, then release Metal buffers.
    # These are fast (~5ms) and must run on main thread.
    torch.mps.synchronize()
    torch.mps.empty_cache()

    # GC runs in background thread — main thread is FREE to grab next batch.
    # By the time the next forward pass completes (~25s), gc is long done.
    if is_deep_clean:
        ...
        logger.info(
            ...
            f"allocated={allocated:.2f} GiB after cleanup"
        )
    elif _cleanup_step_counter % every_n_steps == 0:
