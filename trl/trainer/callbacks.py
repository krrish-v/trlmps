...
class SyncRefModelCallback(TrainerCallback):
    ...
    def _sync_target_model(model, target_model, alpha):
        target_params = [p.data for p in target_model.parameters()]
        model_params = [p.data for p in model.parameters()]
        
        # Dispatch batched kernels sequentially to entirely eliminate CPU python dispatch overhead
        torch._foreach_mul_(target_params, 1.0 - alpha)
        torch._foreach_add_(target_params, model_params, alpha=alpha)
...
class BEMACallback(TrainerCallback):
    ...
    def _update_bema_weights(self, step: int):
        ...
        # Batch unwrap parameters onto the target device
        thetat_list = [t.detach().to(self.device) for t in self.thetat_params]
        
        # Batch EMA update: ema = (1 - beta) * ema + beta * θₜ
        torch._foreach_mul_(self.ema_params, 1 - beta)
        torch._foreach_add_(self.ema_params, thetat_list, alpha=beta)
        
        # Batch BEMA parameter scaling: diff = (θₜ - θ₀)
        diff = torch._foreach_sub(thetat_list, self.theta0_params)
        
        # run_param = ema + alpha * diff
        bema_updates = torch._foreach_add(self.ema_params, diff, alpha=alpha)
        
        # Copy newly scaled matrices back into the executing model memory map
        for run_param, bema_out in zip(self.running_model.parameters(), bema_updates, strict=True):
            run_param.copy_(bema_out)
