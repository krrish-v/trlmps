...
class SyncRefModelCallback(TrainerCallback):
    ...
    def _sync_target_model(model, target_model, alpha):
        ...
        # Collapse the entire parameter sequence iteration into 2 backend CUDA dispatches
...
class BEMACallback(TrainerCallback):
    ...
    def _update_bema_weights(self, step: int):
        ...
        # Compute EMA + BEMA in-place using batched C++ execution to bypass Python looping delay
        thetat_device = [t.detach().to(self.device, non_blocking=True) for t in self.thetat_params]
        
        # EMA update: ema = (1 - beta) * ema + beta * θₜ
        torch._foreach_mul_(self.ema_params, 1 - beta)
        torch._foreach_add_(self.ema_params, thetat_device, alpha=beta)
        
        # BEMA diff: alpha * (θₜ - θ₀)
        diff = torch._foreach_sub(thetat_device, self.theta0_params)
        torch._foreach_mul_(diff, alpha)
        res = torch._foreach_add(self.ema_params, diff)

        # Write directly to running_model parameters
        for run_param, r in zip(self.running_model.parameters(), res, strict=False):
            run_param.copy_(r)
