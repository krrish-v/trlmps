...
class GRPOConfig(BaseConfig):
    ...
    use_mps_optimization: bool = field(
        default=False,
        metadata={
            "help": "Enable Apple Silicon MPS optimizations: sync barriers after generation and logps "
            "computation to prevent async tensor pile-up. No-op on non-MPS systems."
        },
    )
    ...
    mps_fused_loss_chunk_size: int = field(
        default=65536,
        metadata={
            "help": "Vocabulary chunk size for the fused linear + cross-entropy loss. "
            "65536 reduces dispatch overhead to 3 iterations for 152K vocab (Qwen)."
        },
    )
    mps_cleanup_frequency: int = field(
        default=1,
        metadata={
            "help": "How often to run gc.collect() during training (every N steps). gc.collect costs 50-100ms "
            "per call but at 25s/step this is only 0.2-0.4%% overhead. Default 1 (every step) prevents "
            "the ~14MB/step memory leak from autograd reference cycles. Increase only if memory is stable."
        },
    )
