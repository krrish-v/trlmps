...
class GRPOConfig(BaseConfig):
    ...
    mps_fused_loss_chunk_size: int = field(
        default=65536,
        metadata={
            "help": "Vocabulary chunk size for the fused linear + cross-entropy loss. 65536 reduces Python MPS "
            "dispatch latency to prioritize speed while still bounding RAM consumption below PyTorch defaults."
        },
    )
    ...
    mps_cleanup_frequency: int = field(
        ...
        metadata={
            ...
            "Default raised to 10 \u2014 synchronize() now only fires under memory pressure "
    ...
    def __post_init__(self):
        ...
        if self.use_mps_optimization:
            # Deep prefetch buffer: mlx-data uses 2 for natural backpressure
