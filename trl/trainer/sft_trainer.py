...
class SFTTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # Fused CE: runs model backbone (without lm_head) to get hidden_states,
        # then computes CE in vocab chunks. Peak memory: [B, T, chunk_size]
        # instead of [B, T, 152064] (~600MB savings).
        self._use_mps_fused_ce = self._use_mps
    ...
    @staticmethod
    def _find_model_backbone_and_head(model):
        """Extract the transformer backbone and lm_head from any model wrapper.

        Supports: raw model, PEFT-wrapped, DataParallel-wrapped.
        The backbone is the module whose forward produces hidden_states
        (output_hidden_states=True) WITHOUT going through the final lm_head.

        Returns:
            (backbone, lm_head) — both are nn.Module references.

        Raises:
            RuntimeError: If the backbone/lm_head cannot be found.
        """
        from accelerate.utils import is_peft_model

        raw = model.module if hasattr(model, "module") else model
        if is_peft_model(raw):
            raw = raw.get_base_model()

        # Most HF causal LMs use .model for the transformer and .lm_head for the head.
        backbone = getattr(raw, "model", None)
        lm_head = getattr(raw, "lm_head", None)

        if backbone is None or lm_head is None:
            raise RuntimeError(
                "Could not find backbone (.model) and lm_head (.lm_head) on the model. "
                f"Model type: {type(raw).__name__}. "
                "Fused CE requires these attributes. Disable with use_mps_optimization=False."
            )
        return backbone, lm_head

    def _compute_fused_ce_loss(self, model, inputs, labels, mode, num_items_in_batch):
        """Run the fused CE pipeline: backbone → hidden_states → chunked CE.

        This bypasses the lm_head's full-vocab projection, saving ~600MB of
        peak memory for models with large vocabularies (e.g. 152K for Qwen).
        """
        # Run the transformer backbone only (no lm_head projection)
        backbone, lm_head = self._find_model_backbone_and_head(model)

        # Build inputs for backbone (exclude labels — we compute loss ourselves)
        backbone_inputs = {k: v for k, v in inputs.items() if k not in ("labels", "use_cache")}
        backbone_inputs["output_hidden_states"] = True
        backbone_inputs["use_cache"] = False

        backbone_out = backbone(**backbone_inputs)

        # Last hidden state: [B, T, D]
        hidden_states = backbone_out.last_hidden_state if hasattr(backbone_out, "last_hidden_state") else backbone_out[0]

        # Get lm_head weight and bias
        lm_head_weight = lm_head.weight
        lm_head_bias = getattr(lm_head, "bias", None)

        # Shift: predict next token (labels[..., 1:] vs hidden[..., :-1, :])
        shift_hidden = hidden_states[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous()

        # Compute fused CE loss in chunks
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Safety guard: detect all-labels-(-100) batches from broken collators.
        # This costs ~0.1ms and prevents silent zero-loss training.
        if labels is not None and self._use_mps:
            valid_count = (labels != -100).sum()
            if valid_count == 0:
                _counter = getattr(self, "_all_ignore_count", 0)
                if _counter < 5:
                    logger.warning(
                        f"⚠️ All labels are -100 in this batch (shape={labels.shape}). "
                        "Loss will be 0.0. Check your data collator — images may be missing or corrupt."
                    )
                self._all_ignore_count = _counter + 1

        # Fused CE path: bypass lm_head to avoid [B, T, 152064] logit tensor.
        # Only when: MPS fused is enabled, not using liger, during training/eval with loss-only,
        # and no custom compute_loss_func (which expects full outputs).
        _can_use_fused_ce = (
            self._use_mps_fused_ce
    ...
    def training_step(self, *args, **kwargs):
        with self.maybe_activation_offload_context:
            ...
            if self._use_mps:
                # FLOAT32 LOSS CASTING: Prevent bf16 precision drift over 143K steps.
                # MLX-LM casts to float32 BEFORE reduction. Without this, accumulating
                # tr_loss += loss in bf16 causes rounding errors after ~10K steps.
                loss = loss.float()

                # FULL CLEANUP EVERY STEP (including every micro-batch).
                # On Apple Silicon, predictable memory > optimized queuing:
                #   - M4 Pro's unified memory bandwidth makes sync nearly free (~5ms)
                #   - NOT cleaning lets thousands of fragmented memory pointers pile up
                #   - The Metal allocator burns CPU time searching for empty RAM slots
                #   - gc.collect at 50-100ms is only 0.2-0.4% of a 25s step
                # This is the opposite of CUDA best practice, but correct for MPS.
