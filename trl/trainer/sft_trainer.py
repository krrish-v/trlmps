...
from .mps_utils import is_mps_available, mps_synchronize, mps_sync_and_clear, mps_memory_guard, get_mps_device_info, mps_aggressive_cleanup
from .mps_fused_loss import mps_fused_cross_entropy
from .mps_shape_buckets import bucket_pixel_values
...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if self.padding_free:
            ...
            output["labels"].masked_fill_(output["position_ids"] == 0, -100)
        else:
        ...
        if self.completion_only_loss and "completion_mask" in examples[0]:
            ...
            output["labels"].masked_fill_(completion_mask == 0, -100)  # mask everything that is not in the completion
        if "assistant_masks" in examples[0]:
            assistant_masks = pad(
                assistant_masks, padding_value=0, padding_side="right", pad_to_multiple_of=self.pad_to_multiple_of
            )
            output["labels"].masked_fill_(assistant_masks == 0, -100)
        return output
...
class DataCollatorForVisionLanguageModeling(DataCollatorMixin):
    ...
    def _collate_language_modeling(self, examples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        labels.masked_fill_(output["attention_mask"] == 0, -100)
    ...
    def _collate_prompt_completion(self, examples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if self.completion_only_loss:
            labels.masked_fill_(completion_mask == 0, -100)
...
class MPSDynamicBatchCollator(DataCollatorMixin):
    """Token-budget collator inspired by mlx-data's DynamicBatch.

    Instead of using a fixed batch size, this collator packs a variable number
    of samples into each batch so that the total token count stays under
    `max_tokens_per_batch`. Shorter sequences produce larger batches; longer
    sequences produce smaller batches. The total memory footprint per batch
    stays bounded, preventing the RAM spikes that cause swap pile-up on MPS.

    This wraps an existing collator (e.g., DataCollatorForVisionLanguageModeling)
    and controls sample grouping before delegating to the inner collator for
    actual tokenization and padding.

    Based on: mlx-data/mlx/data/buffer/DynamicBatch.cpp
    """

    inner_collator: DataCollator
    max_tokens: int
    return_tensors: str = "pt"

    def __init__(self, inner_collator: DataCollator, max_tokens: int):
    ...
    def torch_call(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        # Estimate token count per sample before full processing.
        # For VLM data, we estimate from message text + image placeholders.
        # For tokenized data, we use input_ids length directly.
        estimates = []
        for f in features:
            if "input_ids" in f:
                estimates.append(len(f["input_ids"]))
            elif "messages" in f:
                total = 0
                for msg in f["messages"]:
                    content = msg.get("content", "")
                    if isinstance(content, str):
                        total += len(content) // 4  # rough char-to-token
                        ...
                        for part in content:
                            if isinstance(part, dict):
                                if part.get("type") == "text":
                                    ...
                                    total += 1000  # typical VLM image token count
                estimates.append(max(total, 64))  # floor at 64 tokens
            elif "prompt" in f and "completion" in f:
                prompt = f["prompt"]
                completion = f["completion"]
                p_len = len(str(prompt)) // 4 if isinstance(prompt, str) else 256
                c_len = len(str(completion)) // 4 if isinstance(completion, str) else 128
                estimates.append(max(p_len + c_len, 64))
            else:
                estimates.append(256)  # fallback estimate

        # Greedy packing: add samples until budget is exceeded.
        # If we can't fit even the first sample, include it anyway (like
        # mlx-data's drop_outliers=False).
        budget_used = 0
        selected = []
        for i, est in enumerate(estimates):
            if budget_used + est > self.max_tokens and len(selected) > 0:
                break  # budget exceeded, stop packing
            selected.append(features[i])
            budget_used += est

        # Delegate to the inner collator for actual processing
        return self.inner_collator(selected)
...
class SFTTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # mlx-data DynamicBatch pattern: wrap collator with token-budget limiter.
        # Instead of fixed batch_size, each batch is capped at max_tokens total tokens.
        # Shorter sequences → larger batches, longer sequences → smaller batches.
        # Total memory per batch stays bounded → no RAM spikes → no swap pile-up.
        mps_max_tokens = getattr(args, 'mps_max_tokens_per_batch', None)
        if mps_max_tokens is not None and self._is_vision_dataset:
            data_collator = MPSDynamicBatchCollator(
                inner_collator=data_collator,
                max_tokens=mps_max_tokens,
            )
            logger.info(
                f"MPS DynamicBatch: wrapping VLM collator with token budget={mps_max_tokens}. "
                f"Batch sizes will vary to keep total tokens per batch bounded."
            )
        ...
        # MPS sorted-by-length batching: pre-compute a "length" column so Transformers'
        # LengthGroupedSampler can group similar-length sequences into the same batch.
        # This reduces padding waste by 30-60% on variable-length VLM data (MLX-LM pattern).
        if getattr(args, 'use_mps_optimization', False) and getattr(args, 'group_by_length', False):
            if "length" not in train_dataset.column_names:
                if "input_ids" in train_dataset.column_names:
                    # Pre-tokenized dataset: exact length from input_ids
                    train_dataset = train_dataset.map(
                        lambda x: {"length": len(x["input_ids"])},
                        num_proc=args.dataset_num_proc,
                        desc="Computing sequence lengths for sorted batching",
                    )
                elif "messages" in train_dataset.column_names:
                    # VLM/chat dataset: estimate length from text content
                            ...
                            if isinstance(msg.get("content"), str):
                                ...
                                for part in msg["content"]:
                                    if isinstance(part, dict) and part.get("type") == "text":
                                        ...
                                        total += 1000  # rough token estimate for images
                        return {"length": total}
                    train_dataset = train_dataset.map(
                        _estimate_length,
                        num_proc=args.dataset_num_proc,
                        desc="Estimating sequence lengths for sorted batching",
                    )
                else:
                    # Fallback: use dataset_text_field
                    text_key = args.dataset_text_field
                    if text_key in train_dataset.column_names:
                        train_dataset = train_dataset.map(
                            lambda x: {"length": len(x[text_key])},
                            num_proc=args.dataset_num_proc,
                            desc="Computing text lengths for sorted batching",
                        )
                logger.info("MPS: pre-computed 'length' column for sorted-by-length batching")
        ...
        # Initialize MPS optimization (Apple Silicon memory management)
        # Only activate if (a) user opted in via config AND (b) MPS hardware is available.
        # On non-MPS systems, _use_mps stays False regardless of the config flag.
        self._use_mps = getattr(args, "use_mps_optimization", False) and is_mps_available()
        self._mps_memory_guard_ctx = None
        self._mps_cleanup_freq = getattr(args, "mps_cleanup_frequency", 4)
        if self._use_mps:
            device_info = get_mps_device_info()
            logger.info(
                f"MPS optimization enabled: sync barriers, shape bucketing, fused loss. "
                f"gc.collect every {self._mps_cleanup_freq} steps. "
                f"Device: {device_info.get('processor', 'unknown')}"
            )
            fraction = getattr(args, "mps_memory_fraction", 0.9)
            self._mps_memory_guard_ctx = mps_memory_guard(fraction)
            self._mps_memory_guard_ctx.__enter__()

            # MetalLiger: fuse RMSNorm + SwiGLU for ~66% memory reduction per op
            if getattr(args, "use_metal_liger", False):
                try:
                    from metal_liger.patch import apply_metal_liger_to_qwen3vl
                    self.model = apply_metal_liger_to_qwen3vl(self.model)
                    logger.info("MetalLiger: fused RMSNorm + SwiGLU active — "
                                "~50% fewer Metal dispatches, ~66% less memory per op")
                except ImportError:
                    logger.warning("MetalLiger: use_metal_liger=True but metal_liger package "
                                   "not found. Install from metal_liger/ directory.")
                except Exception as e:
                    logger.warning(f"MetalLiger: patching failed — {e}. Training continues "
                                   "with standard ops.")

            # Fused CE: compute loss WITHOUT materializing [B, T, 152K] logit tensor (~740MB).
            # Instead, runs model backbone to get last_hidden_state, then uses chunked
            # lm_head matmul + CE that peaks at only [B, T, chunk_size].
            self._use_mps_fused_ce = getattr(args, "use_metal_liger", False)
            self._mps_fused_ce_chunk_size = getattr(args, "mps_fused_loss_chunk_size", 65536)
            if self._use_mps_fused_ce:
                logger.info(
                    f"MPS Fused CE active: lm_head matmul + CE computed in "
                    f"{self._mps_fused_ce_chunk_size}-vocab chunks, saving ~740MB/step"
                )

            # Phase 4: Layer-level torch.compile (PEFT-safe)
            # Compiles each transformer layer.forward individually so PEFT's LoRA
            # adapters are traced as: x @ W_base + (x @ lora_A @ lora_B) * scale
            # = pure linear algebra, no graph breaks, massive dispatch reduction.
            # Also compiles Vision Merger (houses linear_fc1/fc2 LoRA targets).
            # dynamic=False: VLMs have variable visual token counts — dynamic=True
            # triggers silent full recompile on every new image resolution.
            self._metal_liger_compile_result = None
            if getattr(args, "use_metal_liger_compile", False):
                try:
                    from metal_liger._compile import compile_transformer_layers
                    result = compile_transformer_layers(self.model, backend="aot_eager")
                    self._metal_liger_compile_result = result
                    logger.info(
                        f"MetalLiger Phase 4: compiled {result['llm_layers']} LLM layers + "
                        f"{'Vision Merger' if result['merger_compiled'] else 'no merger found'}. "
                        f"Warmup (fwd+bwd) will run before step 1. "
                        f"Run with TORCH_LOGS=recompiles to detect shape-change recompiles."
                    )
                    # Warmup is deferred to train() — needs a real data batch
                    self._metal_liger_needs_warmup = True
                except Exception as e:
                    logger.warning(f"MetalLiger compile: failed — {e}. Continuing without.")
                    self._metal_liger_needs_warmup = False
    ...
    def _find_model_backbone_and_head(self, model):
        """Find the model backbone (without lm_head) and the lm_head weight/bias.

        Handles:
          - PEFT-wrapped: model.base_model.model → has .model (backbone) and .lm_head
          - Standard HF: model → has .model (backbone) and .lm_head
          - Qwen3-VL: model.model is the backbone, model.lm_head is the head

        Returns:
            (backbone_model, lm_head_weight, lm_head_bias)
        """
        ...
        # unwrapped should now be the HF model (e.g., Qwen3VLForConditionalGeneration)
        backbone = unwrapped.model  # Qwen3VLModel (no lm_head)
        lm_head_weight = unwrapped.lm_head.weight
        lm_head_bias = getattr(unwrapped.lm_head, "bias", None)
        return backbone, lm_head_weight, lm_head_bias

    def _compute_fused_ce_loss(self, model, inputs, labels, mode, num_items_in_batch=None):
        """Compute loss using chunked fused lm_head + cross-entropy.

        Instead of materializing the full [B, T, V] logit tensor (~740MB for 152K vocab),
        runs the model backbone to get last_hidden_state, then computes CE in chunks
        of the vocabulary dimension with peak memory of only [B, T, chunk_size].

        This is the MPS equivalent of GRPO's compute_liger_loss path.
        """
        backbone, lm_head_weight, lm_head_bias = self._find_model_backbone_and_head(model)

        # Build backbone inputs (everything except labels — backbone doesn't accept them)
        backbone_inputs = {k: v for k, v in inputs.items() if k not in ("labels", "shift_labels")}
        backbone_inputs["use_cache"] = False  # Ensure cache is disabled for gradient compat

        # Run backbone forward — returns last_hidden_state without lm_head projection
        backbone_outputs = backbone(**backbone_inputs)
        hidden_states = backbone_outputs.last_hidden_state  # [B, T, H]

        # Shift for causal LM: predict next token
        if "shift_labels" in inputs:
            # CP/SP pre-shifted labels
            shift_hidden = hidden_states.contiguous()
            shift_labels = inputs["shift_labels"]
        else:
            shift_hidden = hidden_states[..., :-1, :].contiguous()  # [B, T-1, H]
            shift_labels = labels[..., 1:].contiguous()             # [B, T-1]

        # Compute fused CE loss — never materializes [B, T, V] logits
        loss = mps_fused_cross_entropy(
            hidden_states=shift_hidden,
            lm_head_weight=lm_head_weight,
            labels=shift_labels,
            lm_head_bias=lm_head_bias,
            ignore_index=-100,
            chunk_size=self._mps_fused_ce_chunk_size,
        )

        # Log num_tokens (matching the standard path)
        if mode == "train":
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        # NO SYNC before forward pass — the Metal command buffer stays hot.
        # Forward → backward → optimizer run as one unbroken chain.
        # The ONLY sync happens at the end of training_step via
        # mps_aggressive_cleanup(), which blocks until all GPU work finishes.
        # This simulates MLX's mx.compile: Python fires commands into the
        # Metal command buffer and steps away. GPU executes at hardware speed.

        # Shape bucketing: pad pixel_values to standard sizes to reduce
        # Metal graph recompilation (Wired Memory Leak fix).
        # NOTE: Only 4D/5D spatial dims are padded. 2D/3D (patch embeddings)
        # are passed through because they're tied to image_grid_thw.
        if self._use_mps and "pixel_values" in inputs:
            inputs["pixel_values"] = bucket_pixel_values(inputs["pixel_values"])
        ...
        # ── MPS Fused CE Path ────────────────────────────────────────────
        # When active: runs model backbone (without lm_head) to get
        # last_hidden_state, then computes CE in vocab chunks.
        # Peak memory: [B, T, chunk_size] instead of [B, T, 152064] (~740MB savings).
        #
        # Only activates when:
        #   1. MPS fused CE is configured (_use_mps_fused_ce=True)
        #   2. We're training OR eval is prediction_loss_only (no logits needed)
        #   3. Liger kernel is NOT active (it has its own fused path)
        #   4. We don't need to return outputs (return_outputs=False)
        _can_use_fused_ce = (
            getattr(self, "_use_mps_fused_ce", False)
            and not self.args.use_liger_kernel
            and not return_outputs
            and (mode == "train" or prediction_loss_only)
        )
        if _can_use_fused_ce:
            return self._compute_fused_ce_loss(model, inputs, labels, mode, num_items_in_batch)
        ...
        # Compute entropy — deferred on MPS during training AND during eval when prediction_loss_only.
        # entropy_from_logits + .item() forces a GPU→CPU sync (~10-30ms each).
        # With prediction_loss_only=True, logits are discarded anyway — computing entropy/accuracy
        # on the full 152K-vocab tensor is pure waste (materializes ~600MB per step).
        _should_compute_metrics = not self._use_mps or (mode != "train" and not prediction_loss_only)
        if not self.args.use_liger_kernel and _should_compute_metrics:  # liger doesn't return logits
            with torch.no_grad():
        ...
        elif _should_compute_metrics:
            # Compute accuracy from logits using argmax (traditional method)
            # On MPS during training, this is deferred to avoid GPU→CPU syncs.
            with torch.no_grad():
                if "shift_labels" in inputs:
        ...
        # Early cleanup: release the outputs object (including the logits tensor, ~600MB for
        # Qwen3-VL's 152K vocab) as soon as we're done with it. Without this, the logits tensor
        # stays alive in this scope until compute_loss returns, consuming GPU memory during backward.
        if self._use_mps and not return_outputs:
            del outputs
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        ...
        if getattr(self, "_use_mps", False):
            # Aggressive cleanup during evaluation to prevent MPS swap pile-up
            mps_aggressive_cleanup(
                every_n_steps=self._mps_cleanup_freq,
                deep_clean_interval=100,
            )
            
        return result

    def evaluate(self, *args, **kwargs):
        """Override evaluate to ensure MPS memory is cleanly synchronized and cleared
        before and after evaluation loops, preventing RAM/Swap accumulation.
        """
        if getattr(self, "_use_mps", False):
            import gc
            mps_sync_and_clear()
    ...
    def get_eval_dataloader(self, eval_dataset=None):
        """Override eval dataloader to use 0 workers on MPS.

        During eval, the GPU has no backward pass and runs ~25x faster than training.
        Background dataloader workers add IPC serialization overhead that starves
        the GPU. Direct main-thread feeding is faster.

        IMPORTANT: num_workers must be set BEFORE DataLoader construction.
        Mutating it after instantiation is silently ignored by PyTorch.
        """
        if self._use_mps:
            original_workers = self.args.dataloader_num_workers
            original_prefetch = self.args.dataloader_prefetch_factor
            eval_workers = getattr(self.args, 'mps_eval_num_workers', 0)
            self.args.dataloader_num_workers = eval_workers
            # prefetch_factor is only valid with num_workers > 0;
            # PyTorch raises ValueError if prefetch_factor is set with 0 workers.
            if eval_workers == 0:
                self.args.dataloader_prefetch_factor = None
            dataloader = super().get_eval_dataloader(eval_dataset)
            self.args.dataloader_num_workers = original_workers
            self.args.dataloader_prefetch_factor = original_prefetch
            return dataloader
        return super().get_eval_dataloader(eval_dataset)

    # Override training step to add activation offloading context, MPS sync, and float32 loss.
    def training_step(self, model, inputs, *args, **kwargs):
        # Phase 4 warmup: runs once on the FIRST real batch.
        # Must use a real batch (not dummy) so shapes match exactly.
        # Includes backward pass — aot_eager compiles fwd and bwd graphs separately.
        if getattr(self, "_metal_liger_needs_warmup", False):
            self._metal_liger_needs_warmup = False
            try:
                from metal_liger._compile import warmup_compiled_model
                warmup_compiled_model(model, inputs)
            except Exception as e:
                logger.warning(f"MetalLiger warmup failed — {e}. Step 1 may be slow.")
